// P0010.2.11 C2 — verify the direct getTrend fetch in d7 (近7天) mode:
// 7-day daily series ending TODAY, the data source for the workspace trend card.
...
(async () => {
  ...
  const d7Item = items.find(it => it && it.key==='d7');
  const opt = d7Item.config && d7Item.config.panels ? d7Item.config.panels[0].options[0] : d7Item;
  const resolvedVal = typeof opt.value==='function' ? opt.value(opt) : opt.value;
  const hb = (opt.comparesMap||{}).hb || (opt.compares||[]).find(c=>c&&c.key==='hb') || null;
  const hbResolved = hb && typeof hb.value==='function' ? hb.value(resolvedVal) : (hb ? hb.value : null);
  const M = items.reduce((acc,it)=>{ if (it && it.key) acc[it.key]={min:it.min,max:it.max}; return acc; },{});
  const value = { key:'d7', value: Object.assign({}, opt, { value: resolvedVal }), compareValue: Object.assign({}, hb, { value: hbResolved }) };
  ...
  try {
    ...
    const d0 = resp && resp.body && resp.body.data && resp.body.data[0];
    return { code: resp && resp.header && resp.header.code, bodyKeys: resp && resp.body && Object.keys(resp.body), d0Keys: d0 && Object.keys(d0), d0: JSON.stringify(d0).slice(0, 800) };
  } catch(e) { return { ERR: e.message, params }; }
})()
`);
console.log('RESULT:', JSON.stringify(out, null, 1).slice(0, 2600));
