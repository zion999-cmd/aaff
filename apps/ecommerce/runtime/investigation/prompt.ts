...
export const buildInvestigationPrompt = (
  ...
  return [
    ...
    ``,
    `## Tool Surface — HARD CONSTRAINTS (P0010.2.4 perf)`,
    `You are a constrained business-investigation Agent, not a general coding Agent. The available tool universe in this session is:`,
    ``,
    `**ALLOWED (use these to acquire evidence / read knowledge):**`,
    `- mcp__fabric__fabric_list_capabilities  — discover what live JD data Fabric can provide`,
    `- mcp__fabric__fabric_execute_capability — invoke one capability with the required arguments (shopId, date, etc.)`,
    `- read_file  — ONLY for paths the prompt explicitly names (knowledge/INDEX.md, the specific knowledge page this situation type belongs to, capabilities/INDEX.md, capabilities/<name>.md, the situation's own evidence file). Do not browse the repo.`,
    ``,
    `**DISALLOWED — calling any of these is a turn-failure:**`,
    `- search_files  — do not search the workspace, the repo, or any directory. Read paths you already know.`,
    `- terminal / execute_code / run_command — do not run shell commands. There is no business reason to do so during an investigation.`,
    `- sleep / wait / setTimeout inside a tool call — never block the turn waiting for future data. If a capability is unavailable, emit stopReason="missing_capability" and stop.`,
    `- tool_search / tool_describe / any re-discovery of available tools — the tool inventory above is complete for this session. Calling these wastes turns.`,
    `- reading other situations' case-log / investigation JSON files — stay within this situation. Cross-situation context is delivered by the prompt, not by reading sibling files.`,
    `- exploring the agentFabric repository source code, AGENTS.md, README, or any file under platform/ / apps/ / shared/ / tests/ — the investigation is about a business situation, not about the agentFabric code base.`,
    ``,
    `**Why these rules exist (one line, do not ask follow-ups):**`,
    `On 2026-08-27 a real Investigation turn called search_files 9 times, execute_code once, and ` + "`sleep 60`" + ` + ` + "`sleep 90`" + ` twice — wasting ~150s of the 320s turn. The model itself is fast (~1.6s first delta); the wasted time was the agent's own exploration. Conformance to this section is the single biggest performance lever.`,
    ``,
    `## Investigation Workflow — REQUIRED ORDER`,
    `1. Read situation (in the prompt above) and current evidence. Already loaded — do NOT re-read the situation.`,
    `2. Identify the evidence gap: what is the SINGLE next question that, if answered, reduces uncertainty the most?`,
    `3. If existing evidence already answers it → go to step 7.`,
    `4. Read at most ONE knowledge page relevant to this situation type (via read_file on the path the situation evidence already references; do not search).`,
    `5. If the next question requires live data: call mcp__fabric__fabric_execute_capability ONCE with the best-matching capability. If it fails, retry ONCE. If it still fails, emit stopReason="missing_capability" and stop — do not sleep, do not retry further.`,
    `6. Update hypotheses based on the new evidence.`,
    `7. Form judgment. Emit the canonical Investigation Contract JSON.`,
    ``,
    `**Forbidden pattern**: do NOT inspect repository source code. Do NOT use terminal for waiting. Do NOT call sleep. Do NOT perform generic environment exploration. Do NOT repeatedly rediscover tools already listed above.`,
    ``,
    `## Situation`,
    ...
    ``,
    `Choose the NEXT QUESTION — the single question that, if answered, reduces this situation's uncertainty the most. State its required_evidence (the concrete data you would need). If the Prior Human Guidance already answered it, do NOT ask the same question again — incorporate the answer.`,
    ``,
    `If the next question requires live data, use the Fabric MCP tools (see the Tool Surface section above). The system has a TINY allow-list of equivalents that are normalized to canonical at the boundary. Use ONLY the four strings above; do not emit "confirmed", "strongly_supported", "partially_rejected", or any other variant. The normalization is a safety net, not a license to drift.`,
    ``,
    `Update each hypothesis's status (one of: "proposed", "supported", "weakened", "rejected") and record a finding with evidence_refs.`,
    ``,
    `   ### Status vocabulary — HARD CONSTRAINT`,
    `   `,
    ...
    ``,
    `Decide: continue (choose the next question) or stop. Stop only for one of:`,
    ...
    ``,
    `IMPORTANT: you MUST actually call fabric_execute_capability at least once when existing evidence cannot answer your next question and a matching capability exists. A plausible analysis without evidence acquisition is a failure. But DO NOT call it more than twice for the same capability in the same turn — if both fail, emit stopReason="missing_capability" and stop.`,
    ``,
