...
export const buildInvestigationPrompt = (
  ...
  return [
    ...
    `|---------|-----------|----------------|----------------|`,
    `| **Knowledge** | Long-term professional expertise (rules, methods, cases, SOPs, domain interpretation) | \`knowledge/\` directory (routed from \`knowledge/INDEX.md\`) | To understand HOW to interpret this situation type — see the shared Knowledge section |`,
    ...
    ``,
    `### ALLOWED — Read-only context retrieval`,
    `You may use \`search_files\`, \`read_file\`, \`list_files\` freely within these namespaces only:`,
    `- \`knowledge/\` — professional domain knowledge organized by SEMANTIC DOMAIN (traffic / conversion / product / operations), routed from \`knowledge/INDEX.md\`. How much to read and how to use it: see the shared Knowledge section below.`,
    ...
    `2. **Read relevant Knowledge** — follow the shared Knowledge section: \`knowledge/INDEX.md\` → the matching domain \`INDEX.md\` → the ONE most relevant page. If the situation type is unclear, read \`capabilities/INDEX.md\` to learn what evidence Fabric can supply.`,
