...
export const buildReplayInvestigationPrompt = (
  ...
  return [
    ...
    `- Evidence / knowledge boundaries: \`knowledge/\` is read-only, `,
    `  \`capabilities/\` is read-only. Do NOT call `,
    `  \`mcp__fabric__fabric_execute_capability\` or the browser tools — no LIVE `,
    `  acquisition; the data is already frozen on disk. The ONE permitted tool `,
    `  call is the read-only \`fabric_replay_retrieve_orders\` (see Evidence `,
    `  Resolution); it reads the frozen rows, never acquires and never crosses T.`,
    ``,
    `## Run context`,
    `- run_id: ${run.id}`,
    ...
    ``,
    `Enrichment rules:`,
    `- "fact" / "action" entries are human-supplied records of what objectively happened or was done on the recorded business date. Treat them as dated inputs alongside the frozen evidence; weigh against evidence, cite them explicitly (they are operator-attributed, not system-acquired).`,
    ...
    `- An action followed (on a later date) by an outcome MUST NOT be stated as caused by the action. No causal "action → outcome" claim without evidence beyond temporal order; describe sequence, not causation.`,
    ...
    ``,
    `INVARIANT: A T-1 \`status: proposed\` hypothesis is STILL \`proposed\` at T until NEW Evidence at T shifts it. A T-1 \`status: supported\` hypothesis does NOT auto-upgrade to Confirmed at T. You may write a fresh \`prior_cognition[]\` entry showing how T's new Evidence shifts the T-1 status.`,
    ``,
    ...
    ``,
    `Knowledge is a PROFESSIONAL PRIOR, not current store Evidence. Allowed: \`Knowledge → suggest hypothesis\`. Forbidden: \`Knowledge → manufacture current-world fact\`. If you read a knowledge page that says "8-15 前后常见大促", the correct cognition is hypotheses[] status: proposed with explicit missing_evidence, NOT observed[].`,
    ``,
    ...
    `Quantitative thresholds (e.g. "若 GMV > 12000 则确认") MUST have \`thresholds[]\` with provenance: heuristic | evidence_derived | knowledge_rule | operator_rule | business_policy. heuristic MUST NOT be called a "confirmation rule".`,
    ``,
    ...
    `2. Read the prior_cognition list. What did T-1 already conclude? → keep as prior, do NOT silently re-cite as current.`,
    `3. Form at most 3 hypotheses. Mark each \`proposed\`. Each MUST have \`missing_evidence[]\` + \`falsifier\`.`,
    `4. For the most useful hypothesis, identify the smallest evidence gap. If you use a quantitative threshold, populate \`thresholds[]\` with provenance.`,
    `5. Use the EXISTING evidence, and RESOLVE held evidence before declaring gaps: for any order-structure question (price bands, ex-top1 AOV/GMV, top orders, SKU mix), call fabric_replay_retrieve_orders (it reads frozen rows <= T). Update hypothesis status and cite evidence_refs[]. Examine ALL FIVE business-structure dimensions; a dimension is "gap" ONLY when neither present evidence nor a retrieval can answer it — record the UNAVAILABLE resolution.`,
    `6. Stop per the Analysis Target rule: sufficient structure evidence => business judgment (judgment); unresolved structural gap => missing_capability with evidence_gaps + acquisition_need; observe ONLY when all five dimensions were examined and the reading is normal variation.`,
    `7. Emit the Investigation Contract JSON (shape below) WITH \`epistemic_layers\`, \`claim_evidence_refs\`, \`thresholds\`, \`prior_cognition\`, \`business_structure_coverage\`, \`observed_facts\`, \`supporting_evidence_refs\`, \`evidence_gaps\` populated.`,
    ``,
    ANALYSIS_OUTPUT_OBLIGATIONS,
    ``,
